...
local EdgeTabs = require(UI.EdgeTabs)
...
safeMount("EdgeTabs", function()
    EdgeTabs.mount(context)
end)
safeMount("Menu", function()
    Menu.mount(context)
    -- RIGHT rail: core progression/gameplay tabs.
    EdgeTabs.add("Right", "🔨", "Fusion", function()
        PanelManager.open("Fusion")
    end)
    EdgeTabs.add("Right", "🎽", "Loadout", function()
        PanelManager.open("Loadout")
    end)
    EdgeTabs.add("Right", "🧬", "Evolve", function()
        PanelManager.open("Evolution")
    end)
    EdgeTabs.add("Right", "📜", "Quests", function()
        PanelManager.open("QuestLog")
    end)
    EdgeTabs.add("Right", "🥅", "Net Upgrade", function()
        PanelManager.open("NetShop")
    end)
    EdgeTabs.add("Right", "🤝", "Trade", function()
        PanelManager.open("Trade")
    end)
    EdgeTabs.add("Right", "🛗", "Elevator", function()
        PanelManager.open("Slingshot")
    end)
    -- (Rebirth is NOT an edge tab -- it already lives on the top-left progression rail, no duplicate.)
    -- LEFT rail: just the two high-traffic social/reward tabs (the rest moved into the Menu dropdown
    -- below so the rails stay short + uncluttered instead of overflowing with 7 bubbles).
    EdgeTabs.add("Left", "👥", "Social", function()
        PanelManager.open("Social")
    end)
    EdgeTabs.add("Left", "🎁", "Free Rewards", function()
        PanelManager.open("FreeRewards")
    end)
    -- MENU holds utility/basics + the lower-traffic features (decluttered off the edge rails).
